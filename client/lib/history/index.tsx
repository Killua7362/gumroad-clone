...
const shouldBlockCache: Record<string, boolean> = {};
...
const shouldBlock = async (
    ...
    const routerContext: Router<any, any> = await lastValueFrom(
        getCustomRouteContext
    );
    const nextMatches = routerContext
        .matchRoutes(nextLocation, {}, { throwOnError: false })
        .map((e) => e.routeId);
    const currentMatches = routerContext
        .matchRoutes(currentLocation, {}, { throwOnError: false })
        .map((e) => e.routeId);
    ...
    while (currentRouteId !== '__root__' && nextRouteId !== '__root__') {
        const currentRank: number =
            routerContext.routesById[currentRouteId].rank || 10000;
        const nextRank: number =
            routerContext.routesById[nextRouteId].rank || 10000;
        ...
        if (currentRouteId === nextRouteId) {
            if (layoutUrls.includes(currentRouteId)) {
                ...
                currentRouteId =
                    routerContext.routesById[currentRouteId].parentRoute.id;
                nextRouteId =
                    routerContext.routesById[nextRouteId].parentRoute.id;
            }
...
export function createHistory(opts: {
    ...
    const tryNavigation = async (task: () => void, shouldBlock = true) => {
...
export function createBrowserHistory(opts?: {
    ...
    const layoutUrls: string[] = (opts?.layoutUrls || []).sort();
