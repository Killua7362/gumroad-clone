...
class BrowserHistory {
  ...
  setURL({ to, search }: LinkProps) {
    const prevPages: LinkProps[] = this.homepages.get(
      this.getKey(to as string)
    )!;

    if (!prevPages) return;

    if (prevPages[prevPages.length - 1].to === to) {
      const lastPage: LinkProps[] = this.homepages.get(
        this.getKey(to as string)
      )!;
      lastPage[lastPage.length - 1].search = search;
      this.homepages.set(this.getKey(to as string), [...lastPage]);
    } else {
      this.homepages.set(this.getKey(to as string), [
        ...(prevPages.length > LIMIT ? [prevPages[0]] : prevPages),
        { to, search },
      ]);
    }
  }
