...
const ProductCard = ({ children, productData }: { children: React.ReactNode, productData: ProductCardData }) => {
	return (
		<div className="flex flex-col sm:flex-row gap-y-4 gap-x-6 sm:items-center border-white/30 border-[0.1px] rounded-xl p-6 relative">
			<div>
				{
					productData?.imageSource && productData?.imageSource !== '' ?
						<img src={productData.imageSource!} height={0} width={0} alt='image' className="sm:h-[4rem] sm:w-[4rem] h-[3rem] w-[3rem]" />
						:
