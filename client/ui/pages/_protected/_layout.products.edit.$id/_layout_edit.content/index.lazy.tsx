      ...
      <Fragment>
        <div className="flex justify-between md:justify-start md:flex-col gap-y-3 gap-x-6">
          ...
          <div className="flex flex-col w-fit md:w-full border-white/30 border-[0.1px] h-fit rounded-md p-5 justify-center items-center gap-y-3">
                ...
                {new Array(5).fill(0).map((e, i) => {
                  return (
                    <div
                      ...
                      onMouseLeave={() => {
                        reviewEdit && setTempReviewScore(reviewScore);
                      }}
                      onClick={() => {
                        reviewEdit && setReviewScore(i + 1);
                      }}>
                      {i < tempReviewScore ? <FaStar /> : <CiStar />}
                    </div>
                  );
                })}
              </div>
            ...
            {reviewEdit ? (
              <div>
                <fieldset className="border-white/30 border-[0.1px] rounded-md focus-within:border-white">
                  <textarea
                    className="bg-background text-white outline-none text-base resize-y"
                    value={tempReviewDescription}
                    onChange={(e) => {
            ...
            <Button
              extraClasses={[`!w-full`]}
              buttonName={reviewEdit ? 'Save Changes' : 'Edit Review'}
              onClickHandler={() => {
