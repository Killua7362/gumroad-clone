...
export const allProductsFetcher = ({
  ...
  const { data, isSuccess, isPending } = useQuery({
    ...
    enabled: (queryClient.getQueryData(['loginStatus']) &&
      (queryClient.getQueryData(['loginStatus']) as authSchema).logged_in ==
        true) as boolean,
    initialData,
  });
  ...
  return {
    ...
    queryFn: () => {
      if (queryClient.getQueryData(['allProducts'])) {
        return (
          queryClient.getQueryData(['allProducts']) as ProductTypePayload
        )[productId!];
      } else {
        return fetch(
...
export const getProfileStatusProps = ({
  userId,
}: {
  userId: string | undefined;
}) => {
